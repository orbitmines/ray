...
Parser.prototype.parseULongList = function(count) {
    if (count === undefined) { count = this.u32(); }
...
Parser.prototype.parseUShortList = function(count) {
    if (count === undefined) { count = this.u16(); }
    const offsets = new Array(count);
    const dataView = this.data;
    let offset = this.offset + this.relativeOffset;
    for (let i = 0; i < count; i++) {
...
Parser.prototype.parseValueRecord = function(valueFormat) {
    ...
    if (valueFormat & 0x0001) { valueRecord.xPlacement = this.i16(); }
    if (valueFormat & 0x0002) { valueRecord.yPlacement = this.i16(); }
    if (valueFormat & 0x0004) { valueRecord.xAdvance = this.i16(); }
    if (valueFormat & 0x0008) { valueRecord.yAdvance = this.i16(); }

    // Device table (non-variable font) / VariationIndex table (variable font) not supported
    // https://docs.microsoft.com/fr-fr/typography/opentype/spec/chapter2#devVarIdxTbls
    if (valueFormat & 0x0010) { valueRecord.xPlaDevice = undefined; this.i16(); }
    if (valueFormat & 0x0020) { valueRecord.yPlaDevice = undefined; this.i16(); }
    if (valueFormat & 0x0040) { valueRecord.xAdvDevice = undefined; this.i16(); }
    if (valueFormat & 0x0080) { valueRecord.yAdvDevice = undefined; this.i16(); }
...
Parser.prototype.parseValueRecordList = function() {
    const valueFormat = this.u16();
    const valueCount = this.u16();
...
Parser.prototype.parseListOfLists = function(itemCallback) {
    const offsets = this.parseUShortList();
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (itemCallback) {
            const subOffsets = this.parseUShortList();
...
Parser.byte = Parser.prototype.u8;
Parser.uShort = Parser.offset16 = Parser.prototype.u16;
Parser.uShortList = Parser.prototype.parseUShortList;
Parser.uInt24 = Parser.prototype.parseUInt24;
Parser.uLong = Parser.offset32 = Parser.prototype.u32;
...
Parser.prototype.parseLookupList = function(lookupTableParsers) {
    return this.parsePointer(Parser.list(Parser.pointer(function() {
        const lookupType = this.u16();
        check.argument(1 <= lookupType && lookupType <= 9, 'GPOS/GSUB lookup type ' + lookupType + ' unknown.');
        const lookupFlag = this.u16();
        const useMarkFilteringSet = lookupFlag & 0x10;
        return {
            lookupType: lookupType,
            lookupFlag: lookupFlag,
            subtables: this.parseList(Parser.pointer(lookupTableParsers[lookupType])),
            markFilteringSet: useMarkFilteringSet ? this.u16() : undefined
        };
    }))) || [];
};
...
Parser.prototype.parseFeatureVariationsList = function() {
    return this.parsePointer32(function() {
        const majorVersion = this.u16();
        const minorVersion = this.u16();
...
Parser.prototype.parseItemVariationStore = function() {
    ...
    const variationRegionListOffset = this.u32();
    const itemVariationDataCount = this.u16();
...
Parser.prototype.parseItemVariationSubtable = function() {
    const itemCount = this.u16();
    const wordDeltaCount = this.u16();
...
Parser.prototype.parseDeltaSets = function(itemCount, wordDeltaCount, regionIndexCount) {
    ...
    const wordParser = (longFlag ? this.i32 : this.i16).bind(this);
    const restParser = (longFlag ? this.i16 : this.i8).bind(this);
...
Parser.prototype.parseTupleVariationStoreList = function(axisCount, flavor, glyphs) {
    const glyphCount = this.u16();
    const flags = this.u16();
    const offsetSizeIs32Bit = flags & 0x01;

    const glyphVariationDataArrayOffset = this.u32();
    const parseOffset = (offsetSizeIs32Bit ? this.u32 : this.u16).bind(this);
...
Parser.prototype.parseTupleVariationStore = function(tableOffset, axisCount, flavor, glyphs, glyphIndex) {
    ...
    // header
    const tupleVariationCount = this.u16();
...
Parser.prototype.parseTupleVariationHeader = function(axisCount, flavor) {
    const variationDataSize = this.u16();
    const tupleIndex = this.u16();
...
Parser.prototype.parsePackedPointNumbers = function() {
    ...
    while (points.length < totalPointCount) {
        ...
        for (let i = 0; i < runCount && points.length < totalPointCount; i++) {
            let pointDelta;
            if (numbersAre16Bit) {
                pointDelta = this.u16(); // Parse delta as uint16
            } else {
                pointDelta = this.u8(); // Parse delta as uint8
            }
