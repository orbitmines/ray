...
function parseSTATAxisValue() {
    const valueTableFormat = this.u16();
...
function parseSTATTable(data, start, fvar) {
    ...
    const designAxisSize = p.u16();
    const designAxisCount = p.u16();
    const designAxesOffset = p.u32();
    const axisValueCount = p.u16();
    const offsetToAxisValueOffsets = p.u32();
    const elidedFallbackNameID = (tableVersionMajor > 1 || tableVersionMinor > 0) ? p.u16() : undefined;
