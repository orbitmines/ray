...
export const Ray = new Language('ether', 'E.2026v0.D0')
  ...
  .pass(_ => _
    ...
    .syntax(E => {
      ...
      E.patterned_rule = (node: (_: Language) => Node) => E(E, '{', E.reinterpet('std'), '}', E).length('>=', 1).bind('pattern').freeze().until('=>').block()
        ...
        .interpret(_ => node(_).external_method(_.pattern.map(x => is_string(x) ? x : UNKNOWN), FORWARD_REF))
