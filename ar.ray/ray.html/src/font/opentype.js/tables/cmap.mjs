...
function parseCmapTableFormat12or13(cmap, p, format) {
    ...
    let groupCount;
    cmap.groupCount = groupCount = p.u32();
    cmap.glyphIndexMap = {};

    for (let i = 0; i < groupCount; i += 1) {
        const startCharCode = p.u32();
        const endCharCode = p.u32();
        let startGlyphId = p.u32();
...
function parseCmapTableFormat4(cmap, p, data, start, offset) {
    ...
    let segCount;
    cmap.segCount = segCount = p.u16() >> 1;
    ...
    for (let i = 0; i < segCount - 1; i += 1) {
        let glyphIndex;
        const endCount = endCountParser.u16();
        const startCount = startCountParser.u16();
        const idDelta = idDeltaParser.i16();
        const idRangeOffset = idRangeOffsetParser.u16();
...
function parseCmapTableFormat14(cmap, p) {
    ...
    const numVarSelectorRecords = p.u32();
    ...
    for(let i = 0; i < numVarSelectorRecords; i += 1) {
        ...
        const defaultUVSOffset = p.u32();
        const nonDefaultUVSOffset = p.u32();
