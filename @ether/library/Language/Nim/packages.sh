...
case "$cmd" in
  search)
    ...
    if command -v nimble &>/dev/null; then
      ...
      echo "Visit: https://nimble.directory/search?query=$1"
    fi
    ;;
  info)
    [[ $# -eq 0 ]] && { echo "Usage: $0 info <package>"; exit 1; }
    if command -v nimble &>/dev/null; then
      nimble dump "$1" 2>/dev/null || echo "Visit: https://nimble.directory/pkg/$1"
    else
      echo "Visit: https://nimble.directory/pkg/$1"
    fi
    ;;
  install)
