 ...
 *   2. Earlier alternatives in Token.any()
 *   3. The loop body itself (for next iteration)
 *
 * Only "concrete" tokens (strings, regexes) become boundaries.
 * arbitrary() returns no concrete tokens, so it doesn't block itself.
 *
 * GUARD CONDITIONS:
 *   Use .unless(guard) to add conditions based on preceding content:
 *
 *   Token.string('"').unless(escapedBy('\\'))
 *     - Only match if NOT preceded by odd number of backslashes
 *
 *   Token.string('{').unless(endsWith('\\'))
 *     - Only match if preceding content doesn't end with backslash
 *
 * REPETITION API (Token.times):
 *
 *   Token.times(Token.string(' ')).exactly('indent')
 *     - Match exactly params.indent spaces
 *
 *   Token.times(Token.string(' ')).atLeast(1).bind('added')
 *     - Match 1+ spaces, returns { count, values }, bind to 'added'
 *
 *   Token.times(Token.string('x')).atMost(5)
 *     - Match 0-5 'x's
 *
 *   Token.times(Token.string('y')).between(2, 4)
 *     - Match 2-4 'y's
 *
 *   Token.withParams((ctx, bindings) => ({ indent: ctx.params.indent + bindings.added.count }), ...)
 *     - Compute new params from BOTH current params AND accumulated bindings
 *
 *   Token.end()
 *     - Match end of string
 *
 *   new Parser(grammar, { indent: 0 })
 *     - Initialize params for parsing
 *
 * INDENTATION-SENSITIVE EXAMPLE (unified structure, multiple roots):
 *
 *   // STATEMENT = content + terminator + children (with more indent)
 *   const STATEMENT: Token = Token.array(
 *     Token.arbitrary().bind('content'),
 *     Token.any(Token.string(';'), Token.string('\\n'), Token.end()),
 *     Token.loop(
 *       Token.array(
 *         Token.times(Token.string(' ')).exactly('indent'),     // Parent's indent
 *         Token.times(Token.string(' ')).atLeast(1).bind('added'),  // Additional
 *         Token.withParams(
 ...
 *   );
 *
 *   // PROGRAM = multiple statements at root level
...
class LoopToken extends Token {
  ...
  match(ctx: ParseContext): MatchResult {
    ...
    while (true) {
      ...
      // Check if we should stop: parent boundary OR next iteration could start
      if (iterations >= this.minCount) {
        ...
        // Check if next iteration could start (for loop termination, not passed to children)
        for (const token of innerFirstTokens) {
          if (token.canStartAt(testCtx)) {
            // Don't stop yet - try to match another iteration
            break;
          }
        }
      }
      ...
      // Pass ONLY parent boundaries to children, NOT innerFirstTokens
      const result = innerToken.match({
        input: ctx.input,
        index: currentIndex,
        boundaries: ctx.boundaries,  // <-- CHANGED: don't include innerFirstTokens
        params: ctx.params,
        bindings: ctx.bindings,
      });
...
class AnyToken extends Token {
  ...
  match(ctx: ParseContext): MatchResult {
    ...
    for (let i = 0; i < this.tokens.length; i++) {
      ...
      // Earlier alternatives' concrete tokens are boundaries
      const earlierConcreteTokens: Token[] = [];
      for (let j = 0; j < i; j++) {
        earlierConcreteTokens.push(...this.tokens[j].getFirstConcreteTokens());
      }
      const boundaries = [...earlierConcreteTokens, ...ctx.boundaries];
