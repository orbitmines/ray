...
function parseCFFEncoding(data, start) {
    ...
    const format = parser.u8();
    if (format === 0) {
        const nCodes = parser.u8();
        for (let i = 0; i < nCodes; i += 1) {
            code = parser.u8();
            encoding[code] = i;
        }
    } else if (format === 1) {
        const nRanges = parser.u8();
        code = 1;
        for (let i = 0; i < nRanges; i += 1) {
            const first = parser.u8();
            const nLeft = parser.u8();
...
function parseCFFFDSelect(data, start, nGlyphs, fdArrayCount, version) {
    ...
    if (format === 0) {
        ...
        // Ranges
        const nRanges = format === 4 ? parser.u32() : parser.u16();
        let first = format === 4 ? parser.u32() : parser.u16();
        if (first !== 0) {
            throw new Error(`CFF Table CID Font FDSelect format ${format} range has bad initial GID ${first}`);
        }
        let next;
        for (let iRange = 0; iRange < nRanges; iRange++) {
            fdIndex = format === 4 ? parser.u16() : parser.u8();
            next = format === 4 ? parser.u32() : parser.u16();
            if (fdIndex >= fdArrayCount) {
                throw new Error('CFF table CID Font FDSelect has bad FD index value ' + fdIndex + ' (FD count ' + fdArrayCount + ')');
            }
            if (next > nGlyphs) {
