...
function parseSvgTable(data, offset) {
    ...
    p.relativeOffset = p.u32();
    const svgDocumentListOffset = data.byteOffset + offset + p.relativeOffset;
    const numEntries = p.u16();
    const svgDocMap = new Map();
    for (let i = 0; i < numEntries; i++) {
        const startGlyphID = p.u16();
        const endGlyphID = p.u16();
        const svgDocOffset = svgDocumentListOffset + p.u32();
        const svgDocLength = p.u32();
